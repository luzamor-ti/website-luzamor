...
export async function getPartnersPageData(): Promise<PartnersPageData> {
  const [
    ...
    fetch<SupporterCategory[]>(
      PARTNER_CATEGORIES_QUERY,
      { tipo: "patrocinador", ano: CURRENT_YEAR },
      [],
    ),
    fetch<SupporterCategory[]>(
      PARTNER_CATEGORIES_QUERY,
      { tipo: "apoiador", ano: CURRENT_YEAR },
      [],
    ),
    fetch<SupporterCategory[]>(
      PAST_PARTNER_CATEGORIES_QUERY,
      { tipo: "patrocinador", currentYear: CURRENT_YEAR },
      [],
    ),
    fetch<SupporterCategory[]>(
      PAST_PARTNER_CATEGORIES_QUERY,
      { tipo: "apoiador", currentYear: CURRENT_YEAR },
      [],
    ),
    fetch<Supporter[]>(
      INDIVIDUAL_PARTNERS_QUERY,
      { tipo: "apoiadorIndividual" },
      [],
    ),
